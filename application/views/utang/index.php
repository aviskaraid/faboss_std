...
<div class="container-fluid">
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
	    ...
	    <?php if (is_user()) : ?>
		  	<a href="" class="btn btn-sm btn-primary shadow-sm" data-toggle="modal" data-target="#tambahUtang">
				  	...
				  	<table class="table table-dataAkun table-striped table-bordered table-sm" id="tabel-data">
						...
						<tbody>
				  			...
				  			<?php foreach ($dtUtang as $row) : ?>
						    	<tr>
									<th scope="row"><?= $i++; ?></th>
									<td><?= convertDbdateToDate($row['tgl_invoice']); ?></td>
									<td><?= $row['no_ref']; ?></td>
									...
									<td class="text-center text-nowrap">
										...
										<!-- EDIT -->									      	
										<a href="javascript:void(0)" 
											class="badge badge-warning editUtang"											
											data-toggle="modal" 
											data-target="#editUtang"
											data-id_utang="<?= $row['id_utang'] ?>"
										...
										<!-- DELETE -->
										<a href="#" 
											class="badge badge-danger btn-delete" data-id="<?= $row['id_utang']; ?>" data-tgl_invoice="<?= $row['tgl_invoice']; ?>">
	        ...
	        success: function (res) {
	            ...
	            if ((inputBayar + sudahDibayar) > nilaiAsli) {
	                Swal.fire({
	                    ...
	                    text: 'Total pembayaran melebihi nilai hutang!'
	                });
	                return;
	            }
...
<!-- Modal Tambah Utang  -->
<div class="modal fade" id="tambahUtang" tabindex="-1" role="dialog" aria-labelledby="tambahUtangLabel" aria-hidden="true" >
  	<div class="modal-dialog modal-lg" role="document">
    	<div class="modal-content">
    		<div class="modal-header">
      			<h5 class="modal-title" id="tambahUtangLabel">Tambah Transaksi Hutang</h5>
    		</div>
			<form action="<?= base_url('utang/insert'); ?>" method="post" enctype="multipart/form-data">
	    		<div class="modal-body">
	        		<div class="row">
						<div class="col-sm">
							<div class="form-group row">
							...
							<div class="form-group row">
								...
								<div class="col-sm-9">
									<select name="id_supplier" id="id_supplier" class="form-control bootstrap-select" title="-- Pilih Supplier --" data-width="100%" data-live-search="true" required="">
										<?php foreach($dt_supplier as $row_) : ?>
											<option value="<?= $row_['id_supplier']; ?>">(<?= $row_['kode']; ?>) <?= $row_['nama_supplier']; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
							...
							<div class="form-group row">
								<label for="id_biaya" class="col-sm-3 col-form-label">Akun Pengeluaran</label>
								<div class="col-sm-9">
									<select name="id_biaya" id="id_biaya" class="form-control bootstrap-select" title="-- Pilih Akun Pengeluaran --" data-width="100%" data-live-search="true" required="">
							...
							<div class="form-group row">
								<label for="nilai" class="col-sm-3 col-form-label">Nominal Hutang</label>
								<div class="col-sm-9">
									<input type="text" class="form-control nominal" id="nilai" name="nilai" placeholder="Nominal Hutang" required="">									
								</div>
							</div>
...
<!-- Modal Edit Utang  -->
 <div class="modal fade" id="editUtang" tabindex="-1" role="dialog" aria-labelledby="editUtangLabel" aria-hidden="true" >
  	<div class="modal-dialog modal-lg" role="document">
    	<div class="modal-content">
    		<div class="modal-header">
      			<h5 class="modal-title" id="editUtangLabel">Edit Transaksi Hutang</h5>
    		</div>

			<form action="<?= base_url('utang/update'); ?>" method="post" enctype="multipart/form-data">
	    		<div class="modal-body">
	        		<div class="row">
						<div class="col-sm">
							<input type="hidden" class="form-control" id="id_utang_edit" name="id_utang" required>
							<div class="form-group row">
							...
							<div class="form-group row">
			      				<label for="id_supplier_edit" class="col-sm-3 col-form-label">Supplier</label>
								<div class="col-sm-9">
									<select name="id_supplier" id="id_supplier_edit" class="form-control bootstrap-select" title="-- Pilih Supplier --" data-width="100%" data-live-search="true" required="">
										<?php foreach($dt_supplier as $row_) : ?>
											<option value="<?= $row_['id_supplier']; ?>">(<?= $row_['kode']; ?>) <?= $row_['nama_supplier']; ?></option>
										<?php endforeach; ?>
									</select>
							...
							<div class="form-group row">
								<label for="id_biaya_edit" class="col-sm-3 col-form-label">Akun Pengeluaran</label>
								<div class="col-sm-9">
									<select name="id_biaya" id="id_biaya_edit" class="form-control bootstrap-select" title="-- Pilih Akun Pengeluaran --" data-width="100%" data-live-search="true" required="">
										<?php foreach($dtAkunBiaya as $row_) : ?>
											<option value="<?= $row_['id_biaya']; ?>">(<?= $row_['kode']; ?>) <?= $row_['nm']; ?></option>
										<?php endforeach; ?>
									</select>
							...
							<div class="form-group row">
								<label for="nilai_edit" class="col-sm-3 col-form-label">Nominal Hutang</label>
								<div class="col-sm-9">
									<input type="text" class="form-control nominal" id="nilai_edit" name="nilai" placeholder="Nominal Hutang" required="">									
								</div>
							</div>
...
<script>
// FUNGSI EDIT UTANG - TAMPILKAN DATA KE MODAL
$(document).on('click', '.editUtang', function (e) {
	...
	$("#id_utang_edit").val(id_utang);
    $("#no_ref_edit").val(no_ref);
	$("#id_supplier_edit").val(id_supplier).trigger('change');
    $("#id_biaya_edit").val(id_biaya).trigger('change');
...
$('#editUtang').on('hidden.bs.modal', function () {
    $(this).find('form')[0].reset();
    $('#id_supplier_update').selectpicker('refresh');
    $('#id_biaya_update').selectpicker('refresh');
});
...
$(document).on('click', '#btn-delete-file', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post("<?= base_url('utang/delete_bukti') ?>", { file: fileName }, function () {
...
// Validasi Submit Add Utang
$('#tambahUtang form').on('submit', function (e) {
	e.preventDefault();
...
// Validasi Submit Edit Utang
$('#editUtang form').on('submit', function (e) {
	e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            Swal.fire({
                ...
                text: 'Data Transaksi Hutang yang dihapus tidak bisa dikembalikan!',
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    // 3️⃣ REDIRECT DELETE
                    window.location.href = APP.base_url + 'utang/delete/' + id;
                }
            });
