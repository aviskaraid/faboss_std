...
<div class="container-fluid">
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
	    ...
	    <?php if (is_user() || is_keuangan()) : ?>
		    <a href="" class="btn btn-sm btn-primary shadow-sm" data-toggle="modal" data-target="#tambahTransaksiPendapatan">
									...
									<?php if (is_user() || is_keuangan()) : ?>
										<td class="text-center">
											...
											<a href="javascript:void(0)" 
												class="badge badge-success editTransaksiPendapatan"
												data-toggle="modal" 
												data-id_trans_pendapatan="<?= $row['id_trans_pendapatan'] ?>"
												data-tgl="<?= convertDbdateToDate($row['tgl']) ?>"
												data-keterangan="<?= $row['keterangan'] ?>"
												data-nilai="<?= $row['nilai'] ?>"
												data-kas_id="<?= $row['kas_id'] ?>"
												data-pendapatan_id="<?= $row['pendapatan_id'] ?>"
											...
											<a href="#" 
												...
												data-id="<?= $row['id_trans_pendapatan']; ?>" 
												data-tgl="<?= $row['tgl']; ?>"
												data-posted="<?= $row['posted'] ?>">
...
<!-- Modal Add -->
<div class="modal fade" id="tambahTransaksiPendapatan" tabindex="-1" role="dialog" aria-labelledby="tambahTransaksiPendapatanLabel" aria-hidden="true" >
  	<div class="modal-dialog modal-lg" role="document">
    	<div class="modal-content">
    		<div class="modal-header">
      			<h5 class="modal-title" id="tambahTransaksiPendapatanLabel">Tambah Transaksi Pendapatan</h5>
    		</div>
    		<form action="<?= base_url('transaksipendapatan/insert'); ?>" method="post" enctype="multipart/form-data">
...
<!-- Modal Edit -->
<div class="modal fade" id="editTransaksiPendapatan" tabindex="-1" role="dialog" aria-labelledby="editTransaksiPendapatanLabel" aria-hidden="true" >
  	<div class="modal-dialog modal-lg" role="document">
    	<div class="modal-content">
    		<div class="modal-header">
      			<h5 class="modal-title" id="editTransaksiPendapatanLabel">Edit Transaksi Pendapatan</h5>
    		</div>
    		<form action="<?= base_url('transaksipendapatan/update'); ?>" method="post" enctype="multipart/form-data">
	    		<div class="modal-body">
	        		<div class="row">
						<div class="col-sm">
							<input type="hidden" class="form-control" id="id_trans_pendapatan_update" name="id_trans_pendapatan" required>
			      			<div class="form-group row">
			      				<label for="id_kas" class="col-sm-3 col-form-label">Akun Kas/Bank</label>
								<div class="col-sm-9">
			      			...
			      			<div class="form-group row">
			      				<label for="id_pendapatan" class="col-sm-3 col-form-label">Akun Pendapatan</label>
				    			<div class="col-sm-9">
							    	<select id="id_pendapatan_update" name="id_pendapatan" class="form-control bootstrap-select" title="-- Pilih Akun Pendapatan --" data-width="100%" data-live-search="true" required="">
							    		<?php foreach($dtAkunPendapatan as $row_) : ?>
											<option value="<?= $row_['id_pendapatan']; ?>">(<?= $row_['kode']; ?>) <?= $row_['nm']; ?></option>
										<?php endforeach; ?>
							    	</select>
...
<!-- Modal Delete -->
<div class="modal fade" id="hapusTransaksiPendapatan" tabindex="-1" role="dialog" aria-labelledby="hapusTransaksiPendapatanLabel" aria-hidden="true" >
  	<div class="modal-dialog modal-md" role="document">
    	<div class="modal-content">
      		<div class="modal-header">
        		<h5 class="modal-title" id="hapusTransaksiPendapatanLabel">Konfirmasi Hapus Data</h5>
      		</div>
      		<form action="<?= base_url('transaksipendapatan/delete'); ?>" method="post">
      			<div class="modal-body">
	        		<div class="row">
						<div class="col-sm">
							<input type="hidden" class="form-control" id="id_transaksi_pendapatan_delete" name="id_transaksipendapatan">
...
<div class="modal fade" id="printBiaya" tabindex="-1" role="dialog" aria-labelledby="printLabel" aria-hidden="true" >
  	<div class="modal-dialog" role="document">
    	<div class="modal-content">
			...
			<form action="<?= base_url('transaksipendapatan/print'); ?>" method="post" target="_BLANK">
				<div class="modal-body">				
...
$(document).on('click', '.btn-delete', function (e) {
	...
	if (posted) {
		Swal.fire({
			icon: 'error',
			title: 'Transaksi Sudah Diposting',
			text: 'Transaksi ini sudah diposting dan tidak bisa dihapus'
		});
		return;
	} else {
...
<!-- Edit Transaksi -->
<script>
$(document).on('click', '.editTransaksiPendapatan', function (e) {
    e.preventDefault(); // STOP PAGE RELOAD

    const id_trans_pendapatan = $(this).data('id_trans_pendapatan');
    const tgl           = $(this).data('tgl');
    const keterangan    = $(this).data('keterangan');
    const nilai         = $(this).data('nilai');
    const kas_id        = $(this).data('kas_id');
    const pendapatan_id      = $(this).data('pendapatan_id');
	...
	if (posted) {
		Swal.fire({
			icon: 'error',
			title: 'Transaksi Sudah Diposting',
			text: 'Transaksi ini sudah diposting dan tidak bisa diedit'
		});
		return;
	} else {
		// cek tutup periode
		$.ajax({
			...
			success: function (res) {
				...
				// jika lolos → tampilkan modal edit
				$('#editTransaksiPendapatan').modal('show');
			},
			error: function () {
				Swal.fire('Error', 'Gagal cek periode', 'error');
			}
...
$(document).on('click', '#btn-delete-file', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post("<?= base_url('transaksipendapatan/delete_bukti') ?>", { file: fileName }, function () {
...
<script type="text/javascript">
	...
	$('#tambahTransaksiPendapatan form').on('submit', function (e) {
	    e.preventDefault();
	...
	$('#editTransaksiPendapatan form').on('submit', function (e) {
	    e.preventDefault();
	...
	$(document).ready(function(){
		...
		$('.editTransaksiPendapatan').on('click',function(){
			const id_trans_pendapatan = $(this).data('id_trans_pendapatan');
			const tgl = $(this).data('tgl');
			const keterangan = $(this).data('keterangan');
			const nilai = $(this).data('nilai');
			...
			$("#id_pendapatan_update option[value='" + pendapatan_id + "']").prop("selected", true).trigger('change');
		});

		$('.hapusTransaksiPendapatan').on('click',function(){
			const id_transaksipendapatan = $(this).data('id_transaksipendatan');

			$("#id_transaksipendapatan_delete").val(id_transaksipendapatan);
		});
