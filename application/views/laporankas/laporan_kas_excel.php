...
$sheet->setCellValue('A2', 'Laporan Transaksi '.$nama['nm']); // Set kolom A1 dengan tulisan "DATA SISWA"
...
if (!empty($dbAll))
{
    ...
    foreach ($dbAll as $row)
    {
          ...
          $sheet->setCellValue('D'.$numrow, $row['debit']);
          $sheet->setCellValue('E'.$numrow, $row['credit']);
    ...
    $sheet->setCellValue('D'.$numrow, $total_debit);
    $sheet->setCellValue('E'.$numrow, $total_credit);
...
$sheet->setTitle("Laporan Kas Bank");

// instantiate Xlsx
$writer = new Xlsx($spreadsheet);

$filename = time().'_laporan_kasbank.xlsx';
